...
//        this.setLocationRelativeTo(null);
    ...
    {
        if(x>=0 && x<this.values.length &&y>=0 && y<this.values.length &&values[x][y]==Definitions.EMPTY) {
            return !this.visited[x][y];
        }
        return false;
    }

    public boolean dfsPath(int[][] maze, int x, int y)
    {
        ...
        if (isValid(x, y)) {
            setSquareAsVisited(x,y,true);
            boolean returnValue= dfsPath(maze,x-1,y);
            if(!returnValue)
            {
            ...
            if(!returnValue)
            {
                returnValue = dfsPath(maze,x+1,y);
            }
            if(!returnValue)
            {
                returnValue = dfsPath(maze,x,y-1);
                ...
                for (int i = 0; i < this.visited.length; i++) {
                    for (int j = 0; j < this.visited[i].length; j++) {
                        if (this.visited[i][j]) {
                            ...
                            if((i==this.visited[i].length-1)&&(j==this.visited[j].length-1))
                                 this.buttonList.get(i * this.rows + j).setBackground(Color.GREEN);
                        }
                    }
                }
            } else {
